...
<< uint16(0)
<< uint8(0) // ??
//<< uint8(GetAchieveTitle()) // Achieve Title
//<< uint8(GetAchieveSkillTitle())
//<< uint8(GetAchieveSkillTitle()); // ??
;
#endif
...
void CUser::BundleOpenReq(Packet & pkt)
{
	...
	foreach (itr, pBundle->Items)
		result <<bundle_index<<uint8(2)<< itr->nItemID << itr->sCount;
...
void CUser::HandleHelmet(Packet & pkt)
{
	...
	pkt >> m_bIsHidingHelmet >> m_bIsHidingCospre;
	result<< m_bIsHidingHelmet << m_bIsHidingCospre	<<uint8(0) << uint32(GetSocketID());
	SendToRegion(&result);
}
